...
export default function BtcPriceCard({ price, priceChange }: { price: number, priceChange: number }) {
    return <div className={`${utils.cardBase} ${styles.card}`}>
        <BtcNameSec />
        <BtcPriceSec price={price} priceChange={priceChange} />
        <AnchBtn />
    </div>;
}
...
function BtcPriceSec({ price, priceChange }: { price: number, priceChange: number }) {
    console.log(typeof price, typeof priceChange)
    return <div className={styles.priceContainer}>
        <p className={styles.priceStyle}>${`${price.toLocaleString("en-US", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        })}`}</p>
        <div className={`${styles.priceBadge} ${Number(priceChange) > 0 ? styles.upBadgeCard : styles.downBadgeCard}`}>
